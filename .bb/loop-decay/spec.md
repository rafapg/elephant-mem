...
- [ ] **1. `recall.py` storage and `log`**: script, checkout guard, `log`, `show`, `--at`, and the suite's own `- run:` line in `.github/workflows/ci.yml`, which has no glob
      → E1, E22, E24 · dep: — · verify: `tests/test_recall.py`
- [ ] **2. `roll` and `score`**: pyramid buckets, idempotence, pruning, the lookup decay reads
